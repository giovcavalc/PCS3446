...
vector<pair<string, int>> Montador::carregaPrograma(string programa) {
	...
	if (prog.is_open()) {
		...
		if (campos[0] == "@") {
			...
			if (DEBUG) {
				cout << "Identificando o inicio do programa: @ " << hex
						<< showbase << endereco << endl;
			}
		}
		...
		while (getline(prog, linha)) {
			...
			if ((ch == ' ' || ch == '	')) {
				...
				operando = campos[1];
				if (DEBUG) {
					cout << "OPCODE: " << opcode << " - END: " << endereco
							<< endl;
				}
			} else {
				label = campos[0];
				...
				if (DEBUG) {
					cout << "LABEL: " << label << " - OPCODE: " << opcode
							<< " - END: " << operando << endl;
				}
	...
	if (prog.is_open()) {
		...
		while (getline(prog, linha)) {
			...
			if (campos[0] == "#") {
				...
				buffer << campos[0] << hex << noshowbase
						<< geraOperando(campos[1], tabelaDeNomes);
				pair<string, int> instrucao = make_pair(buffer.str(), endereco);
				instrucoes.push_back(instrucao);

				if (DEBUG) {
					cout << "Identificando o final do programa: "
							<< buffer.str() << endl;
				}
			} else if ((ch == ' ' || ch == '	')) {
				opcode = campos[0];
				operando = campos[1];
				buffer << hex << noshowbase << mnemonicoToHex(opcode)
						<< setfill('0') << setw(3) << right
						<< geraOperando(operando, tabelaDeNomes);
			} else {
				label = campos[0];
				opcode = campos[1];
				operando = campos[2];
